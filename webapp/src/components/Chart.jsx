...
function Chart({ data, selectedChannel, engagementCenters = [] }) {
  ...
  const engagementCenterPoint = useMemo(() => {
    ...
    const maxVideoDistance = data.reduce((maxDistance, item) => {
      ...
      const pointY = item.embedding_2d && item.embedding_2d.length >= 2 ? item.embedding_2d[1] : item.y;

      if (typeof pointX !== 'number' || typeof pointY !== 'number') {
        return maxDistance;
      }

      const pointDistance = Math.sqrt(pointX * pointX + pointY * pointY);
  ...
  }, [showEngagement, selectedChannel, engagementCenters, data]);
